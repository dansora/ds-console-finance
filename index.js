...
function analyzeFinancialRecords(finances) {
  const months = new Set();
  let totalProfitLoss = 0;
  let previousProfitLoss = null;
  let totalChange = 0;
  let greatestIncrease = { date: null, amount: Number.NEGATIVE_INFINITY };
  let greatestDecrease = { date: null, amount: Number.POSITIVE_INFINITY };
  ...
  for (const record of finances) {
    ...
    if (previousProfitLoss !== null) {
      const change = profitLoss - previousProfitLoss;
      totalChange += change;

      if (change > greatestIncrease.amount) {
        greatestIncrease = { dateString, amount: change };
      }

      if (change < greatestDecrease.amount) {
        greatestDecrease = { dateString, amount: change };
      }
    }
